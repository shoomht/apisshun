        ...
        async run({ req, res }) {
            ...
            try {
                ...
                if (isAnimated && typeof result === "object" && "buffer" in result && "contentType" in result) {
                    const __imgResp = createImageResponse(result.buffer, null, result.contentType);
                res.set(__imgResp.headers);
                return res.status(200).send(__imgResp.buffer);
                }
                else if (result instanceof Buffer) {
                    const __imgResp = createImageResponse(result, null, "image/png");
                res.set(__imgResp.headers);
                return res.status(200).send(__imgResp.buffer); // API eksternal return PNG untuk static
                }
                else {
                    return {
                        status: false,

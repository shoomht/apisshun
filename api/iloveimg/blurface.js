...
export default [
    {
        ...
        async run({ req, res }) {
            ...
            try {
                ...
                const __imgResp = createImageResponse(blurredImageBuffer, filename);
    ...
    },
    {
        metode: "POST",
        endpoint: "/api/iloveimg/blurface",
        name: "blurface (upload)",
        category: "Iloveimg",
        description: "Blur faces in an uploaded image. Send multipart/form-data with an image file (field 'file' or 'image').",
        tags: ["ILOVEIMG", "Blur Face", "Privacy", "UPLOAD"],
        upload: true,
        uploadField: "file",
        isPremium: false,
        isMaintenance: false,
        ...
        async run({ req, res }) {
            ...
            try {
                const blurredImageBuffer = await scrapeBlurFaceFromFile(file.buffer, file.originalname || "image.png");
                const fileType = await fileTypeFromBuffer(blurredImageBuffer);
                const __imgResp = createImageResponse(blurredImageBuffer, `blurred_image.${fileType?.ext || "png"}`);
                res.set(__imgResp.headers);
                return res.status(200).send(__imgResp.buffer);
            } catch (error) {
                console.error("Error:", error);

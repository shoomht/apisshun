...
async function downloadCapCutV2(videoUrl) {
    try {
        ...
        const apiResponse = await axios.post('https://anydownloader.com/wp-json/aio-dl/video-data/', `url=${encodeURIComponent(videoUrl)}&token=${token}&hash=${Buffer.from(hash).toString('base64')}`, {
            headers: {
